...
import 'package:Afaq/core/notification_fcm_manager.dart';
import 'package:Afaq/core/provider/all_provider.dart';
...
class XController extends GetxController {
      ...
      _provider.pushResponse('api', jsonBody)!.then((Response? response) {
        if (response != null && response.statusCode == 200) {
          ...
          if (_result['result'] != null && _result['result'].length > 0) {
            dynamic getHome = _result['result'];
  ...
  loadLocalPrefHome() {
    ...
    try {
      ...
      if (pHome.val != '') {
        ...
        dynamic _result = getHome;

        if (_result['category'] != null) {
          List<dynamic>? categs = _result['category'];
          List<CategoryModel> categModels = [];
          if (categs != null && categs.isNotEmpty) {
            for (var e in categs) {
              categModels.add(CategoryModel.fromMap(e));
          ...
          List<dynamic>? latest = _result['latest'];
          List<RentalModel> rentLatestModels = [];
          if (latest != null && latest.isNotEmpty) {
            for (var e in latest) {
              rentLatestModels.add(RentalModel.fromMap(e));
          ...
          List<dynamic>? rents = _result['rent'];
          List<RentalModel> rentModels = [];
          if (rents != null && rents.isNotEmpty) {
            for (var e in rents) {
              rentModels.add(RentalModel.fromMap(e));
          ...
          List<dynamic>? nearbys = _result['nearby'];
          List<RentalModel> rentNearbyModels = [];
          if (nearbys != null && nearbys.isNotEmpty) {
            for (var e in nearbys) {
              rentNearbyModels.add(RentalModel.fromMap(e));
    ...
    try {
      ...
      _provider
        ...
        if (response != null && response.statusCode == 200) {
          ...
          if (_result['result'] != null && _result['result'].length > 0) {
            List<dynamic>? getResults = _result['result'];
  ...
  final photoUser = "".obs;
  updatePhotoUser(String photoUrl) {
    ;
      ...
      if (loggedIn.value) {
        ...
        _provider
          ...
          if (response != null && response.statusCode == 200) {
            ...
            //logPrint(_result);
            if (_result['code'] == '200') {
              List<dynamic>? likes = _result['result']['my_liked'];
              List<RentalModel> rentLikedModels = [];
              if (likes != null && likes.isNotEmpty) {
                for (var e in likes) {
                  rentLikedModels.add(RentalModel.fromMap(e));
              ...
              dynamic getMember = _result['result']['user'];
